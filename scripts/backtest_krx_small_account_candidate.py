#!/usr/bin/env python3
"""Development-only execution test for the pre-registered KRX candidate."""
...
from point_in_time_fundamentals import load_point_in_time_fundamentals
...
def same_timing_kodex200_targets(targets: pd.DataFrame) -> pd.DataFrame:
    """Keep the candidate's risk timing but replace its stock-selection sleeve."""
...
def assert_development_coverage(decisions: pd.DataFrame) -> None:
    """Stop before returns when the pre-registered KRX panel is incomplete."""

    scheduled = decisions.loc[
        decisions["signal_date"].between(DEVELOPMENT_START, DEVELOPMENT_END)
        & decisions["risk_on"]
        & decisions["reason"].eq("krx_snapshot")
    ]
    if scheduled.empty:
        raise RuntimeError("no risk-on KRX development snapshots were evaluated")
    failed = scheduled.loc[
        ~scheduled["snapshots_aligned"].astype(bool)
        | scheduled["fundamental_members"].lt(23)
        | scheduled["index_weight_coverage_pct"].lt(65.0)
    ]
    if not failed.empty:
        diagnostics = failed[
            [
...
def run_development_backtest(
    ...
    *,
    ...
    missing_distribution_audits = sorted(selected - set(stock_distributions))
    # A selected company can legitimately pay no dividend. Its absence from
    # the event mapping is allowed only because the bundle manifest has
    # already audited every selected ticker, including zero-event names.
    ...
    return {
        ...
        "selected_zero_event_tickers": missing_distribution_audits,
