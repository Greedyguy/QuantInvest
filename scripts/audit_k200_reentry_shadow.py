...
def audit_shadow_records(
    ...
    *,
    ...
    prices = _normalize_prices(official_prices, spec)
    ...
    if audit_end is None:
        end = max(eligible_records) if eligible_records else prices.index.max()
    else:
    ...
    expected_dates = prices.loc[start:end].index
    ...
    states = strategy.compute_state_history(prices.loc[:end])
    indicators = strategy._indicators(prices.loc[:end])
    ...
    for day in sorted(record_set & expected_set):
        ...
        prefix = prices.loc[:day]
        row = states.loc[day]
        indicator = indicators.loc[day]
        expected_transition = (
            None
        ...
        checks = {
            "observation_version": record.get("observation_version") == 2,
            ...
            "data_end": record.get("source", {}).get("data_end")
            == day.date().isoformat(),
            "history_start": record.get("source", {}).get("data_start")
            == str(spec["signal_history_start"]),
            "ohlc_prefix_hash": record.get("source", {}).get(
                "causal_ohlc_prefix_sha256"
            )
            == _frame_sha256(prefix),
        ...
        observed = record.get("observable_close_context", {})
        checks["indicator_values"] = all(
            np.isclose(observed.get(key, np.nan), float(indicator[key]))
            for key in (
                "close",
                "trend_ma",
                "fast_ma",
    ...
    account_snapshot = _paper_account_snapshot(
        prices.loc[:end], states.loc[:end], distributions, spec
    )
