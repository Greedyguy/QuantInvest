...
Hybrid Portfolio의 Production Portfolio 전략을 독립적으로 실행
...
class ProductionPortfolioStrategy(BaseStrategy):
    """Production Portfolio 전략 (독립 실행)"""
    
    def __init__(
        self,
        max_positions: int = 6,
        position_size: float = 0.50,  # 50% 포지션 크기
        max_single_stock_ratio: float = 0.20  # 단일 종목 최대 비중
    ):
        """
        Args:
            max_positions: 최대 포지션 수 (기본 6개)
            position_size: 포지션 크기 비율 (기본 50%)
    ...
    def get_description(self) -> str:
        return f"Production Portfolio (최대 {self.max_positions}개 포지션, {self.position_size:.0%} 포지션 크기)"
