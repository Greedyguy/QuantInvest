...
class SectorWeightedStrategy(BaseStrategy):
    """섹터 가중 전략"""
    
    def get_name(self) -> str:
        return "sector_weighted"
    ...
    def run_backtest(self, enriched: dict, market_index=None, weights: dict = None, silent: bool = False) -> tuple:
        ...
        for i in tqdm(range(60, len(dates)-1), desc="Sector Weighted", disable=silent):
        ...
        if not silent:
            print(f"✅ 섹터 가중 백테스트 완료: {len(ec)}개 데이터 포인트\n")
